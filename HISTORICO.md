...
- **Chaves e Credenciais Dinâmicas**: Para evitar o conflito `"invalid access token"` causado quando o administrador altera as credenciais do Mercado Pago no painel mas o frontend continuava usando a Public Key antiga de forma estática (chumbada), criamos a API serverless `GET /api/mercado-pago-public-key` e a função frontend `loadMercadoPagoPublicKey()`. O catálogo público agora carrega a **Public Key atualizada dinamicamente do banco de dados**, alinhando perfeitamente frontend e backend.
