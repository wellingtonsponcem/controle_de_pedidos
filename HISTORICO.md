...
- **API do Mercado Pago corrigida (Fuso Horário & Endpoint)**: Migrada a integração do endpoint legado/restrito de Merchant Orders (`/v1/orders`) para a **API oficial de Pagamentos (`/v1/payments`)** para Pix e Cartão de Crédito transparente. Isso resolveu definitivamente o erro de criação do Pix. Ajustada a expiração do Pix de 4 min no backend para usar explicitamente o **fuso horário local de Brasília (UTC-3)** via offset `-03:00` no payload (`date_of_expiration`). Isso impediu o bug que fazia o Pix vencer imediatamente após a geração. Criada rota `GET /api/pedidos?id={id}` para a entrega.
- **Remoção de Fricção de UX (E-mail Opcional)**: O e-mail do pagador é exigido estritamente pelo Mercado Pago. Para anular de vez qualquer validação de formato nativa do navegador que impedia o envio ou obrigava o usuário, alteramos o input para `type="text"` no HTML (`index.html`, `catalogo.html`). O campo é **100% opcional** no frontend (`catalogo-publico.js`). Se deixado vazio pelo cliente, o backend (`mercado-pago-checkout.ts`) gera de forma transparente um e-mail com base no telefone (`{telefone}@bemavi.local`).
